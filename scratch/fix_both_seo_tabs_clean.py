...
def get_sitedashboardclient_seo():
    return """{/* Tab: SEO (Enterprise Analytics Design) */}
      {activeTab === 'seo' && (
        <div className="space-y-8 animate-in fade-in duration-300">
          {!isGscConnected ? (
...
def apply_sitereporttabs():
    ...
    seo_start = content.find("{/* Tab: SEO (Enterprise Analytics Design) */}")
    if seo_start == -1:
    ...
    new_seo = get_sitereporttabs_seo()
    new_content = content[:seo_start] + new_seo + "\n\n      " + content[seo_end_pos:]
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Updated SiteReportTabs.tsx")
...
def apply_sitedashboardclient():
    ...
    new_seo = get_sitedashboardclient_seo()
    new_content = content[:seo_start] + new_seo + "\n\n      " + content[seo_end_pos:]
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Updated SiteDashboardClient.tsx")

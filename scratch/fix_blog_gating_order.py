def update_site_dashboard_client():
    ...
    tab_blog_idx = content.find("{/* Tab: Blog */}")
    blog_end_idx = content.find("activeTab === 'speed'", tab_blog_idx)
    if blog_end_idx != -1:
        blog_end_idx = content.rfind("}", 0, blog_end_idx)

    new_blog_tab = """{/* Tab: Blog */}
      {activeTab === 'blog' && (
        <div className="space-y-8">
          {(() => {
            const permissions = getBlogPlanPermissions(site)

            // 1. Check if WP is connected
            if (!hasWp) {
              return (
                <div className="bg-white border border-gray-200/80 rounded-[2.5rem] p-12 max-w-2xl mx-auto text-center shadow-sm">
                  ...
                  <p className="text-sm text-gray-500 mb-6">Para poder publicar, agendar e editar artigos deste Blog, a integração do WordPress precisa estar configurada.</p>
                  <Button onClick={() => setActiveTab('geral')} className="bg-black text-white rounded-full px-6 py-3">Voltar para Visão Geral</Button>
                </div>
              )
            }

            // 2. Check Plan Permissions (Gating for Essencial, No Plan or Inadimplente)
    ...
    if tab_blog_idx != -1 and blog_end_idx != -1:
        content = content[:tab_blog_idx] + new_blog_tab + "\n\n      " + content[blog_end_idx:]

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Updated SiteDashboardClient.tsx")
